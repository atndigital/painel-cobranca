"""
banco.py — Persistência via Supabase com fallback local Parquet.
"""
...
def _safe_write(df, path):
    ...
    except Exception as e: print(f"[local] write error: {e}")

def _cv(v):
    """Limpa valor para ser JSON-safe: NaN/inf/NaT → None, date → str."""
    if v is None: return None
    if isinstance(v, float) and (math.isnan(v) or math.isinf(v)): return None
    if isinstance(v, date): return v.strftime('%Y-%m-%d')
    s = str(v)
    if s in ('nan', 'NaT', 'None', '<NA>', 'nat', ''): return None
    return v
    ...
    if sb:
        try:
            ...
            if todos:
                ...
                print(f"[Supabase] controle_envio: {len(df)} registros")
...
def salvar_snapshot(safra, gross, estorno, pagamentos, data=None):
    ...
    if sb:
        try:
            ...
            print(f"[Supabase] snapshot: {safra} {hoje}")
        except Exception as e:
...
# ── HISTÓRICO DE ENVIOS ───────────────────────────────────────────────────────
# Mapeamento colunas painel → Supabase (sem espaços problemáticos)
ETAPA_COL_MAP = {
    ...
    if sb:
        try:
            ...
            if todos:
                df = pd.DataFrame(todos)
                if 'id' in df.columns: df = df.drop(columns=['id'])
                # Renomear colunas do Supabase de volta para exibição
                rename = {'NUMERO_PORTADO': 'NUMERO PORTADO', 'NUMERO_LINHA': 'NUMERO LINHA'}
                df = df.rename(columns=rename)
                _safe_write(df, ENVI_FILE)
                print(f"[Supabase] historico_envios: {len(df)} registros")
                return df
        except Exception as e:
...
def registrar_envios_historico(df_enviados: pd.DataFrame, etapa: str, data_envio: date):
    ...
    col_sb   = ETAPA_COL_MAP.get(etapa, etapa)
    ...
    for _, row in df_enviados.iterrows():
        ...
        novos.append({
            ...
            'SAFRA': safra, 'PORTABILIDADE': str(row.get('PORTABILIDADE','') or ''),
            etapa: data_str,
        })
    if novos:
    ...
    for _, row in df_enviados.iterrows():
        ...
        try:
            ...
            if res.data:
                sb.table('historico_envios').update({col_sb: data_str})\
                  .eq('CPF', cpf).eq('SAFRA', safra).execute()
            else:
                sb.table('historico_envios').insert({
                    'CPF': cpf,
                    'NOME': str(row.get('NOME','') or ''),
                    'NUMERO_PORTADO': str(row.get('NUMERO PORTADO','') or ''),
                    'NUMERO_LINHA':   str(row.get('NUMERO LINHA','') or ''),
                    'SAFRA': safra,
    ...
    print(f"[Supabase] ✓ historico_envios: {ok}/{len(df_enviados)} — {etapa}")


# ── RESUMOS DE SAFRA ──────────────────────────────────────────────────────────
def salvar_resumo(safra: str, resumo: dict):
    """Salva resumo da safra no Supabase como JSON."""
    import json
    from datetime import date
    sb = _get_sb()
    if not sb: return
    try:
        def _fix(v):
            ...
            return str(v)
        resumo_clean = {k: _fix(v) if not isinstance(v, list) else
                       [tuple(_fix(x) for x in t) for t in v]
...
def carregar_resumos() -> dict:
    """Carrega todos os resumos do Supabase."""
    import json
    sb = _get_sb()
    if not sb: return {}
    try:
        res = sb.table('resumos_safra').select('SAFRA,RESUMO_JSON').execute()
        if not res.data: return {}
        resumos = {}
        for row in res.data:
            try:
                r = json.loads(row['RESUMO_JSON'])
                # Restaurar rows como lista de tuplas
                if 'rows' in r and isinstance(r['rows'], list):
                    r['rows'] = [tuple(x) for x in r['rows']]
                # Converter strings numéricas de volta
                for k in ['N','NA','NC','PF','SF','PA','SA','PC_','SC','ET','EP','ES','CT','CP','CS']:
                    if k in r: r[k] = int(float(r[k])) if r[k] is not None else 0
                for k in ['META','MV','RV']:
                    if k in r: r[k] = float(r[k]) if r[k] is not None else 0
                resumos[row['SAFRA']] = r
            except Exception as e:
                print(f"[Supabase] parse resumo {row['SAFRA']}: {e}")
        print(f"[Supabase] ✓ resumos carregados: {list(resumos.keys())}")
        return resumos
    except Exception as e:
        print(f"[Supabase] carregar_resumos: {e}")
        return {}
# ── ATUALIZAÇÃO ───────────────────────────────────────────────────────────────
